...
Ohne Argumente geöffnet – aus dem Finder, dem Launchpad, nach der Installation aus dem App Store
oder TestFlight – zeigt rdp-retina ein Fenster mit vier Bereichen:

| Bereich | |
|---|---|
| Server | beliebig viele RDP-Server: Name, Adresse (`host[:port]`), Benutzer, Domäne, Kennwort (im Schlüsselbund); Status und „Trennen“ |
| RemoteApps | je App Name, Server, Programm, Argumente, Prozessname, Symbol; „Starten“ und „Verknüpfung installieren …“ |
| Übertragung | gleichzeitig offene RemoteApps (0 = unbegrenzt), automatisch neu verbinden samt Versuchen, Ton, Mikrofon, Zwischenablage, alle Bildschirme, Skalierung |
| Arbeitsbereich | offene RemoteApps samt Fensterlage speichern und wiederherstellen |

**Programme eintragen.** Welche Programme ein Server als RemoteApp freigibt, lässt sich über RDP
nicht abfragen – das kann nur der RD-Web-Feed einer RDS-Bereitstellung. Eingetragen wird deshalb,
was in der Freigabeliste des Servers steht: der Alias (`||notepad`) oder der vollständige Pfad.

**Eine Sitzung je Server.** Die Oberfläche verbindet einmal je Server im RemoteApp-Modus ohne
erstes Programm (`/retina-remoteapp`). Programme, die in der Windows-Sitzung noch laufen, erscheinen
dabei sofort. Ist das Programm einer RemoteApp schon offen, kommt sein Fenster nach vorn, statt
dass ein zweites startet.

**Fenster zuordnen.** Windows meldet zu jedem Fenster eine App-ID (MS-RDPERP Get Application ID),
gemessen mit Windows Server 2025:

| Programm | App-ID |
|---|---|
| Editor | `{1AC14E77-02E7-4E5D-B744-2EB1AE5198B7}\notepad.exe` |
| Task-Manager | `Microsoft.AutoGenerated.{923DD477-5846-686B-A659-0FCCD73851A8}` |

Passt der Dateiname – aus dem Programm abgeleitet (`||notepad` → `notepad.exe`) oder unter
„Prozessname“ eingetragen, dort geht auch eine App-ID –, gehört das Fenster zur RemoteApp. Sonst
lernt rdp-retina die App-ID beim ersten Start: Das erste neue, keiner RemoteApp zugeordnete Fenster
nach dem Start gehört dazu.

**Verknüpfungen.** „Verknüpfung installieren …“ legt an einem gewählten Ort (vorgeschlagen
`~/Programme`) eine kleine App mit dem Namen der RemoteApp an. Ihr Symbol ist das hochgeladene Bild
mit dem Symbol von rdp-retina unten rechts. Ein Klick öffnet `rdp-retina://open?app=<ID>`, und
rdp-retina startet die RemoteApp oder holt ihr Fenster nach vorn. Die Verknüpfung ist eine
unveränderte, signierte Kopie von `rdp-retina.app/Contents/Helpers/RDP-Retina Link.app`; die ID
steht im erweiterten Attribut `com.cratchmere.rdp-retina.app`. Ins Dock zieht man sie selbst – eine
App in der Sandbox darf das Dock nicht ändern.

**Arbeitsbereich.** Gespeichert werden je Fenster die RemoteApp, die Lage in Server-Pixeln und der
Zustand (minimiert, maximiert), auf Knopfdruck oder beim Beenden. Öffnet man nach einem Neustart
eine Verknüpfung, deren RemoteApp dazugehört, holt rdp-retina den ganzen Arbeitsbereich zurück:
Schon offene Fenster bekommen ihren Platz, für die übrigen startet das Programm, und sein Fenster
wird beim Erscheinen platziert.

**Anmeldung.** Fehlt ein Kennwort, fragt ein Dialog; angekreuzt landet es nach erfolgreicher
Anmeldung im Schlüsselbund. Ein unbekanntes oder geändertes Serverzertifikat zeigt ein Dialog mit
Fingerabdruck („Vertrauen“, „Nur diesmal“, „Abbrechen“).

**Tastatur.** In einem RemoteApp-Fenster gehen alle Befehlstasten an Windows (Cmd+Q ist dort
Strg+Q); rdp-retina beendet man über das Menü oder das Dock.

**Nicht geprüft.** Die Oberfläche ist ohne Mac entstanden und nirgends kompiliert. Mit dem
Prüf-Client gegen den Testserver gemessen sind die Sitzung ohne erstes Programm, das Erscheinen
noch laufender Fenster, die App-IDs und das Nachstarten über dieselbe Verbindung
(`tests/acceptance/remoteapp-leer.sh`, `multiapp.sh`). Offen ist auch, ob eine Verknüpfung nach dem
Setzen des Symbols noch startet: `setIcon:` schreibt dafür in das kopierte Bundle und bricht das
Ressourcen-Siegel seiner Signatur. Bleibt sie stumm, hilft eine Verknüpfung ohne eigenes Symbol.
...
| `/retina-remoteapp` | RemoteApp-Modus ohne erstes Programm: noch laufende Fenster der Sitzung erscheinen, Programme kommen später dazu (so verbindet die Oberfläche) |
...
anderer Programme; `--createapp` meldet das dort und bricht ab. Dort gibt es stattdessen die
Verknüpfungen der Oberfläche (siehe *Oberfläche*). Der Code bleibt für eine Fassung außerhalb des
App Stores (Developer ID, ohne Sandbox) erhalten.
...
tests/acceptance/multiapp.sh        # mehrere RemoteApps in einer Sitzung
tests/acceptance/remoteapp-leer.sh  # Sitzung ohne erstes Programm und App-IDs (Oberfläche)
```
